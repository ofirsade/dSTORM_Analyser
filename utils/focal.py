...
def plot_FOCAL_res(new_lst, file_name, minL, minC, sig):
    """
    Creates a 3D plot of FOCAL algorithm's results.
    Params:
    ...
    ttl = (file_name + " With Parameters: minL = " + str(minL) +
           " minC = " + str(minC) + " Sigma = " + str(sig))
    sortd = sorted(new_lst, key = lambda x: x[2])
    for point in sortd:
        point[0] = point[0] - 9000 #X: 4000 --> 5500    $$ 9000 --> 11,500
        point[1] = point[1] - 8000 #Y: 15000 --> 18000  $$ 8000 --> 11,000
        point[2] = point[2] + 250 #Z: -650 --> 650       $$ -250 --> 300
...
class Grid:
    ...
    def assign_pts_to_voxels(self, pts_arr):
        ...
        for p in pnt_lst:
            ...
            #add the point into the list of points that are in the voxel [key].
            self.voxel_dict[key][0].append(p)
        ...
        for v in (self.voxel_dict).values(): #Add each localization, with its cluster label to a list of all localizations.
            if (len(v[0]) > 0):
                for p in v[0]:
                    ...
                    full_lst.append(p1)
        no_noise_lst = []
        for p in full_lst: #Add each all clustered localizations to a sub-list.
            if p[3] != -1:
                    ...
                    if voxel_val[2] == False: #If the voxel is not an edge voxel.
                        ...
                        for n in neighbours:
                            if n not in cluster_set:
                                ...
                                ctr += 1
                """
                else: #Check whether this border voxel has any neighbours which are core voxels.
                    if voxel_val[2] == False:
                        neighbours.append(str([vox_coords[0], vox_coords[1], vox_coords[2] + 1]))
                        neighbours.append(str([vox_coords[0], vox_coords[1], vox_coords[2] - 1]))
                        neighbours.append(str([vox_coords[0], vox_coords[1] + 1, vox_coords[2]]))
                        neighbours.append(str([vox_coords[0], vox_coords[1] - 1, vox_coords[2]]))
                        ...
                        for n in neighbours:
                            if n not in cluster_set:
                                if n[3] == True: #If this voxel is a core voxel.
                                    self.rec_vox(n, voxel_dict[n], voxel_dict, cluster_set)
                                    ctr += 1
                """
...
"""
    def calc_cluster_radius(self):
        
        #Calculates the mean and median cluster radii. in 3D or in 2D?
        ...
        for cluster in self.clusters.values():
            ...
            dt = [('x', points.dtype), ('y', points.dtype),('z',points.dtype)]
            assert points.flags['C_CONTIGUOUS']
            a = points.ravel().view(dt)
            a.sort(order=['x','y','z'])
            bl = points[0]
            tr = points[-1]
            dt = [('x', points.dtype), ('y', points.dtype),('z',points.dtype)]
...
def FOCAL(xyz_df, minL, minC, sigma):
    ...
    *********************************************************************************************************************
    @Ret all_locs_df - df, x,y,z coordinates and cluster label for each localisation in the dataset
    ...
    return all_locs_df, clustered_df #all_locs, no_noise_locs
